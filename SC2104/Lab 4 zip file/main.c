...
uint16_t pwmVal=0;       //pwm value to control motor speed
...
int main(void)
{
  ...
  start = 0;
  angle = 0;
  target_angle = 720; // rotate 360 degree
  ...
  while (1)
  {
	  ...
	  if (start==0){ // wait for the User PB to be pressed
    	  pwmVal = 0;
      	  __HAL_TIM_SetCompare(&htim8,TIM_CHANNEL_4,pwmVal);
    	  HAL_GPIO_TogglePin(GPIOE, GPIO_PIN_10); // LED
    	  //OLED_Clear(); // get display ready
    	  OLED_ShowString(15, 40, "Press User"); // show message on OLED display at line 10)
    	  OLED_ShowString(0, 50, "button to start"); // show message on OLED display at line 10)
    	  OLED_Refresh_Gram();
    	  err = 0;// to check whether error has settle down
	      }
	  while (start==0){
    	  HAL_Delay(500);
		  }
	  ...
	  if (abs(error)>2){ //more than 2 degree difference
		  //HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_10); //Buzzer
      	  angle = (int)(position*360/260);  // supposed to be 260 tick per revolution?
    	  error = target_angle - angle;
		  ...
		  pwmVal = error*Kp + error_area*Ki + error*Kd;  // PID

    	  pwmVal = 1000;   // overwrite PID above, minimum pwmVal = 1000
	  ...
	  else{ // error is less than 2 deg
	      err++; // check how long it has reach steady state
	      angle = (int)(position*360/260);  // supposed to be 260 tick per revolution
	      error = target_angle - angle;
	      }
